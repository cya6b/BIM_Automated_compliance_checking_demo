...
function interceptors (axios, Vue, router) {
  ...
  axios.interceptors.response.use(function (response) {
    try {
      if (window.opener && typeof window.opener.reload === 'function') {
        if (response.config.method.toUpperCase() === 'POST') {
          let requestUrl = response.request.responseURL.replace(window.origin, '').replace(/\?.*$/, '')
          window.opener.reload(requestUrl)
        }
      }
      return response
    } catch (err) {
